...
export function CustomConfigModal({
  ...
  const handleOpenChange = (newOpen: boolean) => {
    console.log("CustomConfigModal open state changing to:", newOpen)
    setOpen(newOpen)
    if (onOpenChange) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[700px]">
        <DialogHeader>
          <DialogTitle>Custom View Configuration</DialogTitle>
          <DialogDescription>
            Enter a custom JSON configuration to render a custom view. The configuration must include id, label, entity,
            and columns.
          </DialogDescription>
        </DialogHeader>
