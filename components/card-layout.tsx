...
export function CardLayout({
  ...
  // Get the primary field (first column) for the title
  ...
  // Get the secondary field (second column) for the subtitle
  ...
  // Get the detail fields (remaining columns) for the card body
  const getDetailFields = (item: any) => {
    if (!view.columns || view.columns.length < 3) return []

    // Use all remaining columns, not just a fixed slice
